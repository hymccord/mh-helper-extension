...
export interface Channel {
    addEventListener: (
        listener: (message: MessageEvent<MessageWithMetadata>) => void
    ) => void;
    removeEventListener: (
        listener: (message: MessageEvent<MessageWithMetadata>) => void
    ) => void;
...
export class Messenger {
    private messageEventListener: ((
        event: MessageEvent<MessageWithMetadata>
    ) => void) | null = null;
    ...
    static forDOMCommunication(window: Window) {
        ...
        return new Messenger({
            postMessage: (message, port) =>
                window.postMessage(message, windowOrigin, [port]),
    ...
    private createMessageEventListener() {
        return async (event: MessageEvent<MessageWithMetadata>) => {
            ...
            let onDestroyListener: EventListenerOrEventListenerObject | null = null;
            const destroyPromise = new Promise<never>((_, reject) => {
                onDestroyListener = () => reject(new Error());
            ...
            try {
                ...
                this.onDestroy.removeEventListener(
                    "destroy",
                    onDestroyListener
                );
                port.close();
